...
var displayMap = function displayMap(locations) {
  ...
  mapboxgl.accessToken = "pk.eyJ1Ijoic3BhY2VmYWNlMDIiLCJhIjoiY2txYWlxdHNqMG1qMDJ2bGJhamNqZzVueSJ9.zVp21_X6866ls7XQZRb6WA";
  var map = new mapboxgl.Map({
    container: "map",
    style: "mapbox://styles/spaceface02/ckqaj9t69139s17pixgitvhy0",
    scrollZoom: false // Long first, then lat
  ...
  locations.forEach(function (loc) {
    ...
    markerEl.className = "marker"; // anchor means that the bottom of the pin will be depicting the location.

    new mapboxgl.Marker({
      element: markerEl,
      anchor: "bottom"
    }).setLngLat(loc.coordinates).addTo(map); // Extends the map bounds to include the current location
...
var showAlert = function showAlert(type, message) {
  ...
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\">").concat(message, "</div>\">"); // Inside the body, but right at the beginning.
...
exports.updateUserDataSettings = void 0;
...
var updateUserDataSettings = /*#__PURE__*/function () {
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            url = type === "password" ? "/api/v1/users/updateMyPassword" : "/api/v1/users/updateMe"; // Same server for API and website, hence it works.
          ...
          case 4:
            res = _context.sent;
            if (res.status === 200) (0, _alert.showAlert)("success", "".concat(type.toUpperCase(), " updated successfully! Reload for changes to take place!"));
            _context.next = 11;
            break;
  ...
  return function updateUserDataSettings(_x, _x2) {
    return _ref.apply(this, arguments);
  };
}();

exports.updateUserDataSettings = updateUserDataSettings;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            stripe = Stripe("pk_test_51J718rSHExWYKDwHG56nLf3HDorsuytW4HWQeH9WrZTIEinX7vggyfZ98YUm460PdF9pyvvx1v718EETpFT5bbEg00oEmNav1a"); // 1. Request the session from the server

            _context.next = 4;
            return (0, _axios.default)({
              method: "GET",
              url: "/api/v1/bookings/checkout/".concat(tourId) // withCredentials: true,
...
var loginForm = document.querySelector(".uniqueIdentifierLogin");
...
var bookTourBtn = document.querySelector("#booktour"); // DELEGATION

if (mapbox) {
  // This file gets data from the client and delegates the action to some functions.
  var locations = JSON.parse(document.querySelector("#map").getAttribute("data-locations"));
...
if (updateForm) {
  updateForm.addEventListener("submit", function (e) {
    ...
    var email = document.querySelector("#email").value; // Its an array of files, hence we want only the first one, the only file.

    var photo = document.querySelector("#photo").files[0]; // Lets use the form data API, to create a multipart form-data. Also the enc-type = "multipart/form-data", hence the name.

    var form = new FormData();
    form.append("name", name);
    form.append("email", email);
    form.append("photo", photo);
    e.preventDefault(); // Our AJAX call from axios recognizes this (form) as an object.

    (0, _updateUserData.updateUserDataSettings)(form, "data"); // TODO: FIXME:  Deprecated, so fix this later.

    if (photo !== undefined) (0, _alert.showAlert)("success", "DATA updated successfully! RELOAD for changes to take effect.");
  });
}
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              e.preventDefault();
              document.querySelector(".update-password-button").innerHTML = "Updating..."; // We can't use .value here, as value is only useful when 2 buttons have the same name and we want them to submit different values when clicked. It doesn't change the text on the button in any way.
              ...
              _context.next = 7;
              return (0, _updateUserData.updateUserDataSettings)({
            ...
            case 7:
              ...
              updatePasswordForm.reset(); // Now we have a salt in encrytion, so password with same value are encrypted differently and stored with different values in the database.
